...
topEntity (orangePll12 → (clk12, rst12)) =
  withClockResetEnable clk12 rst12 enableGen top

top ∷
  HiddenClockResetEnable Dom12 ⇒
  Signal Dom12 Bit → Signal Dom12 Bit
top rx = tx
 where
  ...
  result = fmap bitCoerce <$> sictMiSequential @Q @Dom12 toggle dataReg

  bufferStep ::
    (Index (IntegerSize `Div` 8), Vec (IntegerSize `Div` 8) (BitVector 8)) ->
    Maybe (BitVector 8) ->
    ((Index (IntegerSize `Div` 8), Vec (IntegerSize `Div` 8) (BitVector 8)),
     Maybe (Vec (IntegerSize `Div` 8) (BitVector 8)))
