...
module Clash.Crypto.Calculator.Simulate
  ( run
  , traceM
  , CalculatorNum(..)
  , SymbolicNum(..)
  , Fix(..), simplifyFix
  , FixChoice(..), simplifyFixChoice
  ) where
...
instance
  ( KnownInstruction rbound stackSize a i
  , Show k
  , Show a
  , SimulateInstructions (StackParams (i ∷ Instruction k Nat Nat Nat Nat Nat) rbound stackSize a)
  , SimulateInstructions (StackParams is rbound stackSize a)
 ...
 where
  traceInstructionsM _ write simplify as = do
    ...
    as1 ← traceInstructionsM (StackParams i rbound stackSize a) (const $ pure ()) simplify as
    case as1 of
      Nothing   → return Nothing
      Just as2 → do
        let as3 = fmap simplify as2
        ret ← traceInstructionsM (StackParams is rbound stackSize a) write simplify as3
        rnf ret `seq` return ret

instance KnownNat c ⇒ SimulateInstructions (StackParams (PUT c) rbound stackSize a) where
  traceInstructionsM _ _ _ as = pure $ Just (natToNum @c : as)

instance KnownNat n ⇒ SimulateInstructions (StackParams (POP n) rbound stackSize a) where
...
instance KnownNat m ⇒ SimulateInstructions (StackParams (SWP m) rbound stackSize a) where
  traceInstructionsM _ _ _ as
    | i ← natToNum @m
    , i < length as
...
instance KnownNat m ⇒ SimulateInstructions (StackParams (CUP m) rbound stackSize a) where
  traceInstructionsM _ _ _ as
    | i ← natToNum @m
    , i < length as
    = pure $ Just $ (as !! i):as
...
instance
  ...
  , KnownRoutine (r ∷ k)
  , CalculatorNum a
  , forall a' . CalculatorNum a' ⇒ SimulateInstructions (StackParams is rbound' stackSize' a')
  , is ~ Instructions r
  , rbound' ~ RepetitionBound r
  , stackSize' ~ RequiredStackSize r
  , KnownNat n
...
instance
  ( KnownNat p
  , KnownCluInstruction i
  ) ⇒ SimulateInstructions (StackParams (CLU p i) rbound stackSize a)
 where
  traceInstructionsM _ _ _ as = pure $ runOp (natToNum @p) (cluInstruction i) as
...
deriving instance (∀ r . Eq r ⇒ Eq (f r), ∀ r . Eq r ⇒ Eq (g r)) ⇒ Eq (FixChoice f g)
deriving instance Generic (FixChoice f g)
deriving instance (∀ r . NFData r ⇒ NFData (f r), ∀ r . NFData r ⇒ NFData (g r)) ⇒ NFData (FixChoice f g)

-- | Apply a given function at all recursion points of a fixpoint until
-- exhaustion, i.e. until the result no longer changes.
simplifyFix ∷ (∀ r . Eq r ⇒ Eq (f r), Functor f) ⇒ (Fix f → Fix f) → Fix f → Fix f
simplifyFix f x0
  | x' == x0  = x0
