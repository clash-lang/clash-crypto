...
computeModuloUnsigned ∷
  ...
  , 1 ≤ m, ModSize m ≤ len, shifts ~ len - ModSize m, 1 <= shifts + 2
  ) ⇒
  Channel dom (Unsigned len) →
  Channel dom (Mod m)
computeModuloUnsigned = enhance put get compute
 where
  put n = (extend n :: Unsigned (len + 1), maxBound ∷ Index (shifts + 2))
...
computeModuloSigned = enhance put get compute
 where
  put n =
   (signedToUnsigned n :: Unsigned (len + 1), maxBound ∷ Index (shifts + 2), n < 0)
  get _ (a, _, sign) =
   Mod @m . bitCoerce . resize $ if sign && a /= 0 then natToNum @m - a else a
  compute _ (n, j, sign) = ((subIfGE n $ shiftedm j, j - 1, sign), j > 0)
  shiftedm = shiftL (natToNum @m) . fromEnum

-- Add a step from the ECDSA document.
-- Has the restriction than the number must be < modulo^2.
-- TODO: See if Liquid Haskell can ensure this property.
splitNumber ::
 Unsigned (ModSize (CurveModulo SECP256) * 2) ->
 Signed   (ModSize (CurveModulo SECP256) + 7)
splitNumber a = t + s1 * 2 + s2 * 2 + s3 + s4 - id1 - id2 - id3 - id4
 where
  vA :: Vec 16 (Unsigned 32)
  vA = reverse $ bitCoerce a -- To have the right indices.
  fromIndices ::
   Vec 8 (Maybe (Index 16)) -> Signed (ModSize (CurveModulo SECP256) + 7)
  fromIndices = extend . unsignedToSigned . bitCoerce . map (maybe 0 (vA !!))
  t,s1,s2,s3,s4,id1,id2,id3,id4 :: Signed (ModSize (CurveModulo SECP256) + 7)
  t   = fromIndices $(listToVecTH $ fmap Just [7,6,5,4,3,2,1,0 :: Index 16])
  s1  = fromIndices
   $(listToVecTH $ fmap Just [15,14,13,12,11 :: Index 16] <> L.replicate 3 Nothing)
  s2  = fromIndices
   $(listToVecTH $ Nothing : (fmap Just [15,14,13,12 :: Index 16]) <>
                   L.replicate 3 Nothing)
  s3  = fromIndices
   $(listToVecTH $ fmap Just [15,14 :: Index 16] <> L.replicate 3 Nothing <>
                   fmap Just [10,9,8])
  s4  = fromIndices
   $(listToVecTH $ fmap Just [8,13,15,14,13,11,10,9 :: Index 16])
  id1 = fromIndices
   $(listToVecTH $ fmap Just [10,8 :: Index 16] <> L.replicate 3 Nothing <>
                   fmap Just [13,12,11])
  id2 = fromIndices
   $(listToVecTH $ fmap Just [11,9 :: Index 16] <> L.replicate 2 Nothing <>
                   fmap Just [15,14,13,12])
  id3 = fromIndices
   $(listToVecTH $ [Just (12 :: Index 16), Nothing] <> fmap Just [10,9,8,15,14,13])
  id4 = fromIndices
   $(listToVecTH $ [Just (13 :: Index 16), Nothing] <> fmap Just [11,10,9] <>
                   [Nothing] <> fmap Just [15,14])
...
computeModuloPrime =
  case testEquality (natSing :: SNat m) (natSing :: SNat (CurveModulo SECP256)) of
    Just Refl -> computeModuloSigned . fmap splitNumber . delayC
